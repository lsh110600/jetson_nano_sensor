...
initial_set(veml7700, sensor_data_list_veml)
variable_ea = 1

try:
    with conn.cursor() as cur:
        sql = "insert into veml7700 values(%s, %s)"
        while True:
            for _ in range(0, variable_ea):
                globals()['val{}'.format(_)] =  str(select_sensor_data(veml7700, sensor_data_list_veml[_]))
            if val0 is not None:
                print(val0)
                cur.execute(sql, (time.strftime("%Y-%m-%d %H-%M-%S", time.localtime()),val0))
                conn.commit()
            else:
